...
public class SearchService {
    ...
    public List<SearchResponse> search(String q) {
        ...
        StreamSupport.stream(articles.spliterator(), false)
                .map(a -> new SearchResponse(a.getTitle(), ARTICLE_RETRIEVE_URI + a.getId()))
                .forEach(responses::add);

        StreamSupport.stream(user.spliterator(), false)
                .map(a -> new SearchResponse(a.getName(), USER_RETRIEVE_URI + a.getId()))
                .forEach(responses::add);
